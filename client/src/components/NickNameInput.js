...
export const InputHolder = styled.div`
  ...
  color: ${props => props.focus || props.inputFull ? "#4AA1EB" : "#6F767C"};
...
function NickNameInput({nickNameValue, nickNameHandler}) {
  // css 바꿔줄 boolean 값
  ...
  return (
    <InputContainer focus={isFocus} inputFull={nickNameValue.length}>
      <InputHolder focus={isFocus} inputFull={nickNameValue.length}>닉네임</InputHolder>
      <InputUserInfo onKeyUp={(e) => { nickNameHandler(e) }} onFocus={() => { focusHandler() }} onBlur={() => { focusHandler() }} />
    </InputContainer>
  );
}
